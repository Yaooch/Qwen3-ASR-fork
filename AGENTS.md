...
- 训练 `bash finetuning/grpo_train.sh [OUTPUT_DIR] [NPROC] [RESUME]`（默认 4 卡，经 `torchrun` 数据并行：各卡取 rank-strided 样本做 G 路 rollout 与 backward，LoRA 梯度 all-reduce 求平均后同步步进；`apply_lora` 前固定种子使各卡 LoRA 初始化一致），单卡跑传 `NPROC=1`；`--batch_size_per_rank` 控制每卡每步累积样本数，effective batch = NPROC × batch_size_per_rank；`RESUME=1` 从 `OUTPUT_DIR/lora` 续训（恢复 LoRA 权重 + 优化器状态 + step）。评测 `bash finetuning/hotword_eval.sh`（设 `lora` 变量挂载 RL LoRA，留空评基线）。成功标准：热词召回升、误注入降、非热词 CER 不劣化（≤基线 +0.5% 绝对），三者同时达成。
