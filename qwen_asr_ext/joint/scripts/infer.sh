...
usage() {
    ...
    echo "  --wer_script          WER 脚本路径，默认见 qwen_asr_ext/joint/defaults.py"
...
infer_cmd=(
    python3 -m qwen_asr_ext.joint.infer
    --ckpt "${ckpt}"
    --mode "${mode}"
    --input_scp "${input_scp}"
...
for target in "${targets[@]}"; do
    ...
    python3 "${wer_script}" \
        ...
        "${domain_path}" \
        > "${wer_path}"
    python3 -m qwen_asr_ext.evaluation.text_badcase \
        --ref_path "${ref_dir}" \
...
if [[ "${run_pinyin}" -eq 1 ]]; then
    ...
    for target in "${targets[@]}"; do
        ...
        pinyin_cmd=(
            python3 -m qwen_asr_ext.evaluation.pinyin
            --ref_path "${ref_dir}"
            --result_path "${result_path}"
