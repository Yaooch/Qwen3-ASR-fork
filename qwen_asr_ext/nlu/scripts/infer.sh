#!/bin/bash
# qwen_asr_ext/nlu/scripts/infer.sh — joint / 纯 Qwen3 文本 NLU 推理评测
# 用法: BACKEND=joint|llm TASK=nlu|agent EVAL=0|1 bash qwen_asr_ext/nlu/scripts/infer.sh [INPUT] [GPU_IDS]
set -euo pipefail
...
python3 -m qwen_asr_ext.nlu.infer \
    --backend "$backend" \
    --ckpt "$ckpt" \
