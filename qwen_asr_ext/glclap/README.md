...
| 模型 | V1～V4：Data2Vec Audio Large + multilingual BERT；V5：Qwen3-ASR-1.7B Audio Encoder + multilingual BERT |
...
| V5 | Qwen Audio Encoder / Text Encoder 全量微调 | 4 × 16 | 10k | 与 V4 相同 | 200k，训练完成 |
...
V5 用官方 Qwen3-ASR-1.7B 初始化，使用 `train_mask` Encoder 输出（左 24 帧、当前 8 帧、右 0 帧）。全量训练从 Mel 到 `ln_post` 的 Audio Encoder、mBERT、两个 512 维投影层和温度参数，不经过 Qwen `proj1/act/proj2`。损失和 subtext 采样与 V4 相同。训练 200k steps，projection LR `1e-3`，Encoder LR `1e-5`，global batch 64。
...
| GLCLAP V5 200k | **78.00% / 91.80%** | **79.94% / 91.63%** | 78.65% / 96.80% | 90.96% / 98.59% |
...
V5 相比 V4，STOP1 Top-1 / Top-3 提升 7.89 / 6.33 个百分点，STOP2 提升 12.66 / 6.87 个百分点；AISHELL 恢复到接近 V3 160k 的水平，但 Voyah Top-1 比 V3 160k 低 5.11 个百分点。
...
| GLCLAP V5 200k（不含 Encoder） | 0.611 | 0.600 | 0.714 | 0.587 |

V5 以完整 `ln_post` Encoder 输出为计时起点，只统计 1024→512 投影、候选相似度和 Top-3，不包含特征提取和 Qwen Audio Encoder，因此不能与 V3/V4 的耗时直接对比。V5 这次评测未设阈值，每条固定返回 3 个候选。
...
- V5：`/cfs/data/private/WangYaoChi/model/glclap/retrieval_v5/final`
- CTC：`/cfs/data/private/WangYaoChi/test_out/joint_ctc_50_grpo_2/asr_hotword`

V3 / V4 的统一评测明细位于各版本目录下的 `eval_{stop1,stop2,aishell,voyah}_checkpoint-*_top3_latency.jsonl`。V5 明细位于 `retrieval_v5/eval_{stop1,stop2,aishell,voyah}_final_top3_latency.jsonl`。
